...
<div class="container bg-form p-4 m-5">
    <form class="text-white " enctype="multipart/form-data" action="{{ route('restaurants.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('name') is-invalid @elseif(old('name')) is-valid @enderror" name="name" value="{{ $errors->has('name') ? '' : old('name') }}">

            <div class="mb-3">
                <label class="form-label">Numero di telefono</label>
                <input type="number" class="form-control @error('phone_number') is-invalid @elseif(old('phone_number')) is-valid @enderror" name="phone_number" value="{{ $errors->has('phone_number') ? '' : old('phone_number') }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('vat') is-invalid @elseif(old('vat')) is-valid @enderror" name="vat" value="{{ $errors->has('vat') ? '' : old('vat') }}">
                </div>

                <div class="mb-3">
                    <label class="form-label">Indirizzo</label>
                    <input type="text" class="form-control @error('address') is-invalid @elseif(old('address')) is-valid @enderror" name="address" value="{{ $errors->has('address') ? '' : old('address') }}">
                </div>
